...
# If you estimate additional observation errors in your model, good news! 
# You can now visualize how the additional estimated observation error relates 
# to the assumed observation error based on the design-based estimator.   

# Documentation:
# The original issue is here: https://github.com/afsc-assessments/rema/issues/14
# A reproducible example is in this section of the "Fitting to an additional CPUE survey" 
# vignette: https://afsc-assessments.github.io/rema/articles/ex2_cpue.html#model-2-
# estimating-additional-observation-error-in-the-two-surveys
...
#                          dependencies = TRUE, 
#                          build_vignettes = TRUE) #add force=TRUE if that doesn't work.
# This will not affect any other functionality of the rema package.
#  Don't worry about your existing code breaking.


# Example R scripts are downloaded when `rema` is installed. 
# Locate them on your computer by running the following commands:
...
bio <- read.csv("data/SEO_YE_Biomass_subdistrict_2024-10-28.csv")
...
#ind <-ind %>% filter(CPUE == "Tweedie index") %>% 
#select(strata = SEdist, year = Year, cpue, cv)
...
# IPHC standardized index using the sdmTMB model
ind_raw <- read.csv(paste0(here::here(), "/outputs/bridging analysis/IPHC.cpue.SEO_sdmTMB_m1_m2_m3_17July2026.csv"))
...
plots22_1 <- plot_rema(output22_1, biomass_ylab = 'ROV biomass', 
...
# Model estimates and clean data frames..
output22_2 <- tidy_rema(m22_2)  

names(output22_2)

print(output22_2$total_predicted_biomass,n = 50)
...
# Get the built in plots
plots22_2 <- plot_rema(output22_2, 
                       biomass_ylab = 'ROV biomass', 
                       cpue_ylab = 'IPHC setline survey CPUE')

# Biomass estimates by strata
plots22_2$biomass_by_strata

# IPHC CPUE estimates by strata
...
# Save the plots for the SAFE
ggsave(file.path(paste0(here::here(), "/figures/",YEAR, "/REMA/biomass_by_strata_22_2.png")), 
       plot = plots22_2$biomass_by_strata,
       height = 0.6*7,
       width = 7, 
       units = "in")

ggsave(file.path(paste0(here::here(), "/figures/",YEAR, "/REMA/cpue_by_strata_22_2.png")), 
       plot = plots22_2$cpue_by_strata,
       height = 0.6*7, 
       width = 7, 
       units = "in")

ggsave(file.path(paste0(here::here(), "/figures/",YEAR, "/REMA/total_predicted_biomass_22_2.png")), 
       plot = plots22_2$total_predicted_biomass,
       height = 0.6*7, 
       width = 7, 
       units = "in")


write.csv(output22_2$total_predicted_biomass, 
          file = paste0(here::here(), "/outputs/",YEAR, "/REMA/REMA_total_predicted_biomass_22_2_sdmTMB_2026.csv"))
...
ggsave(p1, file = paste0(here::here(), "/figures/",YEAR, "/REMA/adfg_survey_fits_22_2_sdmTMB.png"), 
       width = 6, 
       height = 4, 
       units = "in", 
       bg = "white")
...
ggsave(p2, file = paste0(here::here(), "/figures/",YEAR, "/REMA/iphc_survey_fits_22_2_sdmTMB.png"), 
       width = 6, 
       height = 4, 
       units = "in", 
       bg = "white")
...
ggsave(p3, file = paste0(here::here(), "/figures/",YEAR, "/REMA/est_biomass_22_2_sdmTMB.png"), 
       width = 6, 
       height = 4, 
       units = "in", 
       bg = "white")
...
ggsave(p4, file = paste0(here::here(), "/figures/",YEAR, "/REMA/fits_22_2_sdmTMB.png"), 
       width = 10, 
       height = 4, 
       units = "in",
       bg = "white")
...
sur.1.plot <- ggplot(sur.1, aes(x = year, y = std_index, color = index)) +
  ...
  scale_color_manual(values = c(cbpalette[2], cbpalette[3]), name = "") +
  xlab("Year") +
...
ggsave(plot = sur.1.plot, 
       filename = paste0(here::here(), "/figures/",YEAR, "/REMA/stand_indices.png"), 
       width = 7, 
...
ggsave(plot = sur.1.plot.pres, 
       filename = paste0(here::here(), "/figures/",YEAR, "/REMA/stand_indices_pres.png"), 
       width = 9, 
       height = 7, units = "in",
       bg = "white")
...
biomass.prev <- read.csv(paste0(here::here(), "/outputs/2026/cpue_2024_model_digitized.csv")) 

compare.prev <- output22_2$total_predicted_biomass %>%
  mutate(model = "22.2 - 2026") %>%
  select(model, year, pred, pred_lci, pred_uci) %>%
  rbind(biomass.prev %>%
          mutate(model = "22.2 - 2024") %>% 
          select(model, year, pred = estimate, pred_lci = lower, pred_uci = upper))
...
ggsave(cp.plot, file = paste0(here::here(), "/Figures/2026/est_biomass_compare.png"), 
       width = 6, height = 4, units = "in", bg = "white")

ggsave(cp.plot, file = paste0(here::here(), "/Figures/2026/est_biomass_compare_pres.png"), 
...
cpue.prev <- read.csv(paste0(here::here(), "/outputs/REMA_total_predicted_biomass_22_2_formatted.csv")) %>%
  rename("CPUE" = "pred") %>%
  mutate(model = "22.2 - 2024") %>%
...
write.csv(OFL_ABC, file = paste0(here::here(), "/outputs/2026/REMA/REMA_ref_pts_22_2.csv"))
...
write.csv(par.est, file = paste0(here::here(), "/outputs/2026/REMA/REMA_param_est_22_2_sdmTMB.csv"))
